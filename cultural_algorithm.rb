...
# Modified by Tosidis Dimitrios (January 2015) in order to be used in "iris.data"

require 'csv'

$arr_of_arrs = CSV.read("iris.data")

$i=0

def get_population()
  $i +=1
  return [$arr_of_arrs[$i-1][0].gsub(/[^\d^\.]/, '').to_f, $arr_of_arrs[$i-1][1].gsub(/[^\d^\.]/, '').to_f, 
$arr_of_arrs[$i-1][2].gsub(/[^\d^\.]/, '').to_f, $arr_of_arrs[$i-1][3].gsub(/[^\d^\.]/, '').to_f]
end

def objective_function(vector)
  return vector.inject(0.0) {|sum, x| sum +  x}
end
...
def search(max_gens, search_space, pop_size, num_accepted)
  # initialize
  pop = Array.new($arr_of_arrs.size-1){{:vector => get_population()}}
...
if __FILE__ == $0
  ...
  problem_size = 4
  search_space=[[4.3, 7.9],[2.0, 4.4],[1.0, 6.9], [0.1, 2.5]]
